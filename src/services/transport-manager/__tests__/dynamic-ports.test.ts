...
describe('Transport Manager Dynamic Port Allocation', () => {
  let testServers: import('net').Server[] = [];
  ...
  describe('Graceful Degradation', () => {
    it('should continue with available transports when some fail', async () => {
      ...
      (websocketServer.start as Mock).mockRejectedValueOnce(new Error('WebSocket startup failed'));
    ...
    it('should handle all network services failing gracefully', async () => {
      ...
      (websocketServer.start as Mock).mockRejectedValue(new Error('WebSocket failed'));
      (httpAgentAPI.start as Mock).mockRejectedValue(new Error('HTTP failed'));
  ...
  describe('Service Retry Logic', () => {
    it('should retry service startup with alternative ports', async () => {
      ...
      expect((websocketServer.start as Mock).mock.calls.length).toBeGreaterThanOrEqual(2);
    });

    it('should give up after maximum retries', async () => {
      // Mock service to always fail
      const { websocketServer } = await import('../../websocket-server/index.js');
      (websocketServer.start as Mock).mockRejectedValue(new Error('Always fails'));
      ...
      expect((websocketServer.start as Mock).mock.calls.length).toBeGreaterThan(1);
    });
  });
