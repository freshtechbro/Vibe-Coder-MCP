...
export class DependencyContainer {
  ...
  private async initializeAgentRegistry(): Promise<unknown | null> {
    try {
      const registryModule = await ImportCycleBreaker.safeImport<{ AgentRegistry: { new(): unknown; getInstance(): any } }>('../tools/agent-registry/index.js');
  ...
  private async initializeAgentTaskQueue(): Promise<unknown | null> {
    try {
      const taskQueueModule = await ImportCycleBreaker.safeImport<{ AgentTaskQueue: { new(): unknown; getInstance(): any } }>('../tools/agent-tasks/index.js');
  ...
  private async initializeAgentResponseProcessor(): Promise<unknown | null> {
    try {
      const responseModule = await ImportCycleBreaker.safeImport<{ AgentResponseProcessor: { new(): unknown; getInstance(): any } }>('../tools/agent-response/index.js');
  ...
  private async initializeAgentIntegrationBridge(): Promise<unknown | null> {
    try {
      const bridgeModule = await ImportCycleBreaker.safeImport<{ AgentIntegrationBridge: { new(): unknown; getInstance(): any } }>('../tools/vibe-task-manager/services/agent-integration-bridge.js');
