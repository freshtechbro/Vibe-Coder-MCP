 ...
 */
function detectOperationType(requestData: unknown): LLMOperationType {
  const messages = (requestData as Record<string, unknown>)?.messages || [];
  const systemMessage = (messages as Array<Record<string, unknown>>).find((m: Record<string, unknown>) => m.role === 'system')?.content || '';
  const userMessage = (messages as Array<Record<string, unknown>>).find((m: Record<string, unknown>) => m.role === 'user')?.content || '';
 ...
 */
function formatResponseForOperation(content: unknown, operationType: LLMOperationType): string {
