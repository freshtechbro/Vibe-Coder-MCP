...
export default defineConfig(({ mode }) => {
  ...
  return {
    test: {
      ...
      // CI tests are limited to 30s to ensure fast pipeline execution
      // Tests exceeding this limit should be moved to comprehensive test suites
      testTimeout: process.env.TEST_TYPE === 'unit' ? 5000 : 
                   process.env.TEST_TYPE === 'integration' ? 60000 :
                   process.env.TEST_TYPE === 'ci' ? 30000 : // Strict 30s limit for CI
                   15000,
